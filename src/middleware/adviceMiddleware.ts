...
export const areYouTheAdviceOwnerOrTheAdvicer = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const adviceId = req.params.adviceId
        const advice = await getOneAdviceById(new mongoose.Types.ObjectId(adviceId.trim()))
        if (advice) {
            const plant = await getOnePlantById(advice.plant._id)
            if (plant) {
                if (req.user._id.equals(plant.user._id) || req.user._id.equals(advice.taked_by?._id) ) {
                    next()
                } else {
                    res.status(404).send({ message: "Your are not allowed" });
                }
...
export const isThisAdviceTaken = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const adviceId = req.params.adviceId
        const advice = await getOneAdviceById(new mongoose.Types.ObjectId(adviceId.trim()))
        if (advice) {
