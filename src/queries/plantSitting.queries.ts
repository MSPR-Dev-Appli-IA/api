...
import {PlantQueries} from "./plant.queries";
...
export const findOnePlantSittinWithRequest = async (plantSittingId: Types.ObjectId): Promise<IPlantSitting> => {
...
export class PlantSittingQueries{
    private messageRepository: MessageQueries;
    private requestRepository: RequestQueries;
    private plantRepository: PlantQueries;
    constructor() {
        this.messageRepository = new MessageQueries()
        this.requestRepository = new RequestQueries()
        this.plantRepository = new PlantQueries()
    }

    public async findById (plantSittingId: string): Promise<IPlantSitting> {
    ...
    public async getOneByRequestId(requestId: string){
    ...
    public async rollBack(requestId: string){
        const currentPlantSitting = await this.getOneByRequestId(requestId)
        const ownerInfo = await this.plantRepository.findById(currentPlantSitting.plant._id.toString())
        ...
        await this.messageRepository.createMessageForRequest(
        ...
        if(currentPlantSitting.requests.length > 0){
            for(let i = 0; i < currentPlantSitting.requests.length; i++){
                const newRequestInfo = await this.requestRepository.findById(currentPlantSitting.requests[i]._id.toString())
    ...
    public async linkPlantSittingToRequest (plantSitting: IPlantSitting, requestId: string)  {
        const requestInfo = await this.requestRepository.findById(requestId)
        const plantSittingInfo = await this.findById(plantSitting._id.toString())
