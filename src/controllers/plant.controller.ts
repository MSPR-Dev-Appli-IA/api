...
import {findLimitedPlantsByUserIdAndSpeciesId,findOnePlant,createPlant,addImageWithPlantId,deleteImageWithPlantId ,deletePlantsWithPlantsId} from "../queries/plant.queries"
...
export const getMyPlants = async (req: Request, res: Response, _: NextFunction) => {
    try {
        let { page=1, order,speciedId } = req.body;
        order = order == "ASC" ? 1 :-1
        const skip:number =  limit * page - limit;
        const plants = await findLimitedPlantsByUserIdAndSpeciesId(req.user._id,speciedId,limit,skip,order)
...
export const newPlant = async (req: Request, res: Response, _: NextFunction) => {
    try {
        let { speciedId } = req.body;
        const species = await findOneSpecies(speciedId)
        if (species){
            const newPlant = createPlant(species._id,req.user._id)
